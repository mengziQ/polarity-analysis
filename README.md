...
ツイート（とツイートに関する付加情報）は、json形式で取得できます。取得したjsonファイルから、ツイートの部分のみを取得し、    
時間×ユーザーごとにテキストファイルに格納しました。    
このとき、泣いている絵文字「😢」が入っているツイートには、ファイル名に「\_n」を、笑っている絵文字「😃」が入っているツイートには、  
「\_p」を付加しました。  
...
また、わかち書きの処理は「MeCab」という形態素解析エンジンを使用しました。（参考コード：wakatiAndEmoji.py）  
...
わかち書きにした形態素にIDを付与します。pickle形式で出力します。pickle形式なので中身を見ることはできませんが、  
出力ファイルのイメージは以下です。（参考コード：giveId.py）  
...
分析に影響をきたす場合のみ使用します。（参考コード：giveFreq.py）  
...
上位10ワードずつ出力します。（参考コード：outTop10.py）  
<br />
結果は以下のようになりました。
